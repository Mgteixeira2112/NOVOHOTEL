...
test('workspace factory creates a routed operational definition', () => {
  ...
  assert.equal(workspace.widgets.find(widget => widget.type === 'kanban-cards')?.boardId, 'kanban-board-recepcao');
});

test('workspace factory keeps metrics and kanban on the selected board', () => {
  ...
  assert.equal(moved.widgets.find(widget => widget.type === 'kanban-cards')?.boardId, 'kanban-board-manutencao');
});
