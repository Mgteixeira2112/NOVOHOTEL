...
export const GovernancaCardDetailModal: React.FC<{
  ...
  useEffect(() => {
    ...
    setResponsibleId(card ? getGovernancaAssignedUserId(card) : '');
  }, [card?.id, card?.assigned_to, (card as any)?.assigned_user_id]);
  ...
  const activeUsers = users.filter(user => user.ativo).sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR'));
  ...
  const changeResponsible = async (nextUserId: string) => {
    if (!currentUser?.id || savingResponsible) return;
    setResponsibleId(nextUserId);
    setSavingResponsible(true);
    try {
      const selected = activeUsers.find(user => user.id === nextUserId);
      const assignedTo = selected ? {
        id: selected.id,
        name: selected.nome,
        email: selected.email,
        avatar_url: selected.avatar_url,
        role: selected.tipo_usuario,
      } : null;
      await kanbanCardGovernance.updateCard(card, { assigned_to: assignedTo }, { userId: currentUser.id });
    } finally {
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/60 p-3 sm:p-6" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl">
      <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur">
        ...
        <button disabled={busy || savingResponsible} onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 disabled:opacity-40"><X className="h-4 w-4" /></button>
      </div>
      ...
      <div className="grid gap-4 p-5 md:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><h3 className="text-xs font-black uppercase tracking-wide text-slate-500">Informações</h3><div className="mt-3 space-y-3 text-xs"><div className="flex items-center gap-3"><DoorClosed className="h-4 w-4 text-slate-400" /><div><p className="text-[10px] text-slate-400">Quarto / Local</p><strong>{card.room_number ? `Quarto ${card.room_number}` : card.location || 'Não informado'}</strong></div></div><div className="flex items-start gap-3"><User className="mt-2 h-4 w-4 text-slate-400" /><div className="min-w-0 flex-1"><p className="text-[10px] text-slate-400">Responsável</p><strong>{getGovernancaAssignedName(card) || 'Sem responsável'}</strong>{currentUser?.id && <select disabled={savingResponsible || busy} value={responsibleId} onChange={event => void changeResponsible(event.target.value)} className="mt-2 h-9 w-full rounded-xl border border-slate-200 bg-white px-2 text-xs font-bold text-slate-700 disabled:opacity-50"><option value="">Sem responsável</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.nome}</option>)}</select>}<p className="mt-1 text-[9px] text-slate-400">Qualquer usuário autenticado pode trocar o responsável por enquanto.</p></div></div><div className="flex items-center gap-3"><CalendarClock className="h-4 w-4 text-slate-400" /><div><p className="text-[10px] text-slate-400">Criado / atualizado</p><strong>{formatDate(card.created_at)}</strong><p className="text-[10px] text-slate-400">{formatDate(card.updated_at)}</p></div></div></div></section>
