...
const roomStatusLabel = (status?: string | null) => ({
  disponivel: 'Disponível', ocupado: 'Ocupado', manutencao: 'Manutenção', sujo: 'Sujo', limpeza: 'Em limpeza', em_limpeza: 'Em limpeza',
  vistoria: 'Vistoria', aguardando_vistoria: 'Aguardando vistoria', inspecionado: 'Inspecionado', aprovado: 'Aprovado', limpo: 'Limpo',
  bloqueado: 'Bloqueado', nao_perturbe: 'Não perturbe',
...
const roomStatusTone = (status?: string | null) => {
  ...
  if (key === 'ocupado') return 'border-blue-200 bg-blue-50 text-blue-700';
...
export const GovernancaWorkCenterInsights: React.FC<{ governanceCards: KanbanV2Card[] }> = ({ governanceCards }) => {
  const { rooms, reservations, guests, currentUser, updateRoom, hasTabAccess, getRoleModulePermission } = useHotel();
  ...
  const governanceCardFor = (room: Quarto) => governanceCards
    .filter(card => String(card.room_number || '') === String(room.numero) && !card.is_archived)
    .sort((a, b) => new Date(b.updated_at || b.created_at || 0).getTime() - new Date(a.updated_at || a.created_at || 0).getTime())[0];
  const kanbanHousekeepingStatus = (room: Quarto) => {
    const card = governanceCardFor(room);
    if (!card) return room.status_governanca || room.status_housekeeping || null;
    ...
    return room.status_governanca || room.status_housekeeping || null;
  };
  const requiresAttention = (room: Quarto) => {
    const operational = normalize(room.status);
    const housekeeping = normalize(kanbanHousekeepingStatus(room));
  ...
  const roomCounts = useMemo(() => ({
    ...
    limpeza: rooms.filter(room => ['limpeza', 'em_limpeza', 'sujo'].includes(normalize(kanbanHousekeepingStatus(room) || room.status))).length,
    vistoria: rooms.filter(room => ['vistoria', 'aguardando_vistoria'].includes(normalize(kanbanHousekeepingStatus(room) || room.status))).length,
  ...
  }), [rooms, governanceCards]);

  const filteredRooms = useMemo(() => {
    const query = normalize(roomQuery);
    return rooms.filter(room => {
      const housekeeping = kanbanHousekeepingStatus(room);
      if (query && ![room.numero, room.nome, room.descricao, room.status, housekeeping, room.status_manutencao_motivo].filter(Boolean).join(' ').toLowerCase().includes(query)) return false;
      if (roomFilter === 'all') return true;
      if (roomFilter === 'attention') return requiresAttention(room);
      if (roomFilter === 'limpeza') return ['limpeza', 'em_limpeza', 'sujo'].includes(normalize(housekeeping || room.status));
      if (roomFilter === 'vistoria') return ['vistoria', 'aguardando_vistoria'].includes(normalize(housekeeping || room.status));
      return normalize(room.status) === roomFilter;
    });
  }, [rooms, governanceCards, roomFilter, roomQuery]);
  ...
  const changeHousekeeping = async (room: Quarto, status: string) => {
    if (!canEditRooms || roomActionBusy || !currentUser?.id) return;
    setRoomActionBusy(true);
    setRoomActionMessage('');
    try {
      const targetColumn = status === 'em_limpeza'
        ? GOVERNANCA_STAGES.working
        : status === 'aguardando_vistoria'
          ? GOVERNANCA_STAGES.inspection
          : status === 'aprovado'
            ? GOVERNANCA_STAGES.done
            : null;
      const linkedCard = governanceCardFor(room);
      if (!linkedCard || !targetColumn) {
        setRoomActionMessage(`Não existe card operacional da Governança vinculado ao quarto ${room.numero}.`);
        return;
      }
      const moved = await kanbanCardGovernance.moveCard(linkedCard, targetColumn, { userId: currentUser.id });
      setAllCards(current => upsert(current, moved));
      ...
      setRoomActionMessage(`Quarto ${room.numero} e Kanban sincronizados em ${roomStatusLabel(status)}.`);
    } catch (error: any) {
      setRoomActionMessage(error?.message || 'Não foi possível sincronizar o quarto com o Kanban.');
    } finally {
  ...
  const requestMaintenance = async (room: Quarto) => {
    ...
    setRoomActionMessage('');
    try {
      await createGovernancaDemand({ sector: 'manutencao', title: `Verificar quarto ${room.numero}`, description: room.status_manutencao_motivo || `Solicitação operacional criada a partir do painel do quarto ${room.numero}.`, roomNumber: room.numero, priority: 'alta', actorUserId: currentUser.id });
  ...
  const roomFilterOptions: Array<{ key: RoomFilter; label: string; count: number }> = [
    { key: 'all', label: 'Todos', count: roomCounts.total }, { key: 'attention', label: 'Atenção', count: roomCounts.attention },
    { key: 'disponivel', label: 'Disponíveis', count: roomCounts.disponivel }, { key: 'ocupado', label: 'Ocupados', count: roomCounts.ocupado },
    { key: 'limpeza', label: 'Limpeza', count: roomCounts.limpeza }, { key: 'vistoria', label: 'Vistoria', count: roomCounts.vistoria },
    { key: 'manutencao', label: 'Manutenção', count: roomCounts.manutencao }, { key: 'bloqueado', label: 'Bloqueados', count: roomCounts.bloqueado },
  ];

  return <>
    <div className="grid gap-2 sm:grid-cols-3">{cards.map(item => <button key={item.kind} onClick={() => setActive(item.kind)} className="group flex min-h-[74px] items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-left transition hover:border-amber-300 hover:bg-amber-50/50"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-slate-600 shadow-sm"><item.icon className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="flex items-baseline gap-2"><strong className="text-xs text-slate-800">{item.label}</strong><b className="text-lg text-slate-950">{item.value}</b></span><span className="block truncate text-[10px] text-slate-500">{item.detail}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-slate-300 transition group-hover:text-amber-600" /></button>)}</div>

    {active && <div className="fixed inset-0 z-[75] flex items-center justify-center bg-slate-950/55 p-3 sm:p-6" onMouseDown={event => { if (event.target === event.currentTarget) { setActive(null); setSelectedRoom(null); } }}><div className="max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl"><div className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur"><div><p className="text-[10px] font-black uppercase tracking-wider text-amber-600">Central de trabalho · Tempo real</p><h2 className="text-lg font-black">{active === 'maintenance' ? 'Manutenção' : active === 'rooms' ? 'Painel operacional de quartos' : 'Alertas do setor'}</h2></div><button onClick={() => { setActive(null); setSelectedRoom(null); }} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200"><X className="h-4 w-4" /></button></div><div className="p-5">
      ...
      {active === 'rooms' && canViewRooms && <div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">{roomFilterOptions.map(option => <button key={option.key} onClick={() => setRoomFilter(option.key)} className={`rounded-2xl border px-3 py-2.5 text-left transition ${roomFilter === option.key ? 'border-slate-950 bg-slate-950 text-white' : 'border-slate-200 bg-slate-50 text-slate-700 hover:border-amber-300'}`}><span className="block text-[9px] font-black uppercase tracking-wide opacity-70">{option.label}</span><span className="mt-1 block text-xl font-black">{option.count}</span></button>)}</div><div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><label className="relative block w-full max-w-xl"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={roomQuery} onChange={event => setRoomQuery(event.target.value)} placeholder="Buscar quarto, status ou observação" className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-xs outline-none focus:border-amber-300" /></label><span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-400"><ShieldCheck className="h-3.5 w-3.5" />Ações conforme permissões de {currentUser?.nome || 'usuário'}</span></div>{roomActionMessage && <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-[10px] font-bold text-blue-700">{roomActionMessage}</div>}<div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filteredRooms.map(room => { const housekeeping = kanbanHousekeepingStatus(room); const currentGuest = guestNameFor(room.id); const openMaintenance = maintenanceFor(room); const attention = requiresAttention(room); return <button key={room.id} onClick={() => { setSelectedRoom(room); setRoomActionMessage(''); }} className={`group rounded-2xl border bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${attention ? 'border-amber-200' : 'border-slate-200'}`}><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><strong className="text-sm text-slate-950">Quarto {room.numero}</strong>{attention && <AlertTriangle className="h-3.5 w-3.5 text-amber-500" />}</div><p className="mt-0.5 text-[10px] text-slate-400">{room.nome || `Andar ${room.andar}`}</p></div><span className={`rounded-lg border px-2 py-1 text-[9px] font-black ${roomStatusTone(room.status)}`}>{roomStatusLabel(room.status)}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-xl bg-slate-50 p-2.5"><span className="text-slate-400">Governança</span><p className="mt-1 truncate font-black text-slate-700">{roomStatusLabel(housekeeping)}</p></div><div className="rounded-xl bg-slate-50 p-2.5"><span className="text-slate-400">Hóspede</span><p className="mt-1 truncate font-black text-slate-700">{currentGuest || '—'}</p></div></div>{openMaintenance.length > 0 && <div className="mt-2 flex items-center gap-2 rounded-xl border border-rose-100 bg-rose-50 px-2.5 py-2 text-[9px] font-black text-rose-700"><Wrench className="h-3.5 w-3.5" />{openMaintenance.length} chamado(s) de manutenção em aberto</div>}<div className="mt-3 flex items-center justify-between text-[9px]"><span className="text-slate-400">Última limpeza: {room.ultima_limpeza ? new Date(room.ultima_limpeza).toLocaleDateString('pt-BR') : 'não informada'}</span><span className="font-black text-amber-700">Abrir operação →</span></div></button>; })}</div>{filteredRooms.length === 0 && <div className="mt-4 rounded-2xl border-2 border-dashed border-slate-200 p-10 text-center"><BedDouble className="mx-auto h-6 w-6 text-slate-300" /><p className="mt-2 text-xs font-bold text-slate-400">Nenhum quarto encontrado neste filtro.</p></div>}</div>}
    ...
    {selectedRoom && <div className="fixed inset-0 z-[85] flex items-center justify-center bg-slate-950/60 p-3 sm:p-6" onMouseDown={event => { if (event.target === event.currentTarget) setSelectedRoom(null); }}><div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur"><div><p className="text-[10px] font-black uppercase tracking-wider text-amber-600">Operação do quarto</p><div className="mt-1 flex flex-wrap items-center gap-2"><h3 className="text-xl font-black">Quarto {selectedRoom.numero}</h3><span className={`rounded-lg border px-2 py-1 text-[9px] font-black ${roomStatusTone(selectedRoom.status)}`}>{roomStatusLabel(selectedRoom.status)}</span></div></div><button onClick={() => setSelectedRoom(null)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200"><X className="h-4 w-4" /></button></div><div className="space-y-4 p-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl bg-slate-50 p-3"><p className="text-[9px] font-black uppercase text-slate-400">Governança</p><p className="mt-1 text-xs font-black text-slate-800">{roomStatusLabel(kanbanHousekeepingStatus(selectedRoom))}</p></div><div className="rounded-2xl bg-slate-50 p-3"><p className="text-[9px] font-black uppercase text-slate-400">Responsável</p><p className="mt-1 text-xs font-black text-slate-800">{selectedRoom.responsavel_limpeza || 'Não informado'}</p></div><div className="rounded-2xl bg-slate-50 p-3"><p className="text-[9px] font-black uppercase text-slate-400">Última limpeza</p><p className="mt-1 text-xs font-black text-slate-800">{formatDateTime(selectedRoom.ultima_limpeza)}</p></div><div className="rounded-2xl bg-slate-50 p-3"><p className="text-[9px] font-black uppercase text-slate-400">Fechadura</p><p className="mt-1 text-xs font-black text-slate-800">{selectedRoom.fechadura_bateria != null ? `${selectedRoom.fechadura_bateria}% bateria` : 'Não informado'}</p></div></div>
      {governanceCardFor(selectedRoom) && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-[10px] font-bold text-emerald-700">Kanban conectado em tempo real · etapa atual: {columnName(governanceCardFor(selectedRoom)!)}</div>}
      ...
      <div className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black text-slate-900">Ações operacionais</p><p className="mt-0.5 text-[10px] text-slate-400">Os comandos disponíveis são determinados pelas permissões do perfil.</p></div><ShieldCheck className="h-5 w-5 text-slate-400" /></div><div className="mt-3 flex flex-wrap gap-2">{canEditRooms && <><button disabled={roomActionBusy} onClick={() => void changeHousekeeping(selectedRoom, 'em_limpeza')} className="h-9 rounded-xl bg-slate-950 px-3 text-[10px] font-black text-white disabled:opacity-40">Iniciar limpeza</button><button disabled={roomActionBusy} onClick={() => void changeHousekeeping(selectedRoom, 'aguardando_vistoria')} className="h-9 rounded-xl bg-amber-400 px-3 text-[10px] font-black text-slate-950 disabled:opacity-40">Enviar para vistoria</button><button disabled={roomActionBusy} onClick={() => void changeHousekeeping(selectedRoom, 'aprovado')} className="flex h-9 items-center gap-1.5 rounded-xl bg-emerald-600 px-3 text-[10px] font-black text-white disabled:opacity-40"><CheckCircle2 className="h-3.5 w-3.5" />Aprovar limpeza</button></>}{canRequestMaintenance && <button disabled={roomActionBusy} onClick={() => void requestMaintenance(selectedRoom)} className="flex h-9 items-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-3 text-[10px] font-black text-rose-700 disabled:opacity-40"><Wrench className="h-3.5 w-3.5" />Solicitar manutenção</button>}{!canEditRooms && !canRequestMaintenance && <span className="inline-flex items-center gap-2 rounded-xl bg-slate-100 px-3 py-2 text-[10px] font-bold text-slate-500"><ShieldCheck className="h-3.5 w-3.5" />Visualização somente leitura</span>}</div>{roomActionMessage && <p className="mt-3 rounded-xl bg-blue-50 px-3 py-2 text-[10px] font-bold text-blue-700">{roomActionMessage}</p>}</div>
      <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4"><div className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-slate-400" /><p className="text-[10px] font-black uppercase text-slate-500">Leitura operacional</p></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><div className="rounded-xl bg-white p-3"><Sparkles className="h-4 w-4 text-amber-500" /><p className="mt-2 text-[10px] font-bold text-slate-600">Governança e Kanban agora usam o mesmo status operacional.</p></div><div className="rounded-xl bg-white p-3"><Wrench className="h-4 w-4 text-rose-500" /><p className="mt-2 text-[10px] font-bold text-slate-600">Chamados de manutenção são vinculados ao número do quarto.</p></div><div className="rounded-xl bg-white p-3"><BellRing className="h-4 w-4 text-blue-500" /><p className="mt-2 text-[10px] font-bold text-slate-600">Mudanças do Kanban são refletidas em tempo real nesta tela.</p></div></div></div>
    </div></div></div>}
  </>;
};