...
export const GovernancaWorkspace: React.FC<{ definition: WorkspaceDefinition }> = ({ definition }) => {
  ...
  const createDemand = async (draft: GovernancaDemandDraft) => {
    ...
    try {
      const created = await createGovernancaDemand({ ...draft, sourceCard: demandSourceCard, actorUserId: currentUser.id });
      if (created.board_id === boardId && !created.is_archived) setCards(current => current.some(item => item.id === created.id) ? current : [...current, created]);
      if (sourceCardId(created)) setRelatedDemands(current => current.some(item => item.id === created.id) ? current.map(item => item.id === created.id ? created : item) : [...current, created]);
      setDemandOpen(false); setDemandSourceCard(null);
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-950">
    ...
    <main className="max-w-[1600px] mx-auto p-4 sm:p-6 space-y-5">
      ...
      {kanbanEnabled && <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">{columns.map(column => { const columnCards = visibleCards.filter(card => card.column_id === column.id); return <div key={column.id} className="min-h-[360px] rounded-3xl border border-slate-200 bg-slate-50 p-3"><div className="mb-3 flex items-center justify-between rounded-2xl bg-white px-3 py-2.5"><div><p className="text-[10px] font-black uppercase tracking-wide text-amber-700">Governança</p><h3 className="text-xs font-black text-slate-800">{column.nome}</h3></div><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-500">{columnCards.length}</span></div><div className="space-y-3">{columnCards.map(card => { const demands = demandsFor(card.id); return <article key={card.id} role="button" tabIndex={0} onClick={() => setSelectedCard(card)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') setSelectedCard(card); }} className="cursor-pointer rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-md"><div className="flex items-start justify-between gap-2"><div><p className="text-[9px] font-black uppercase text-slate-400">{stageLabel(card.column_id)}</p><h4 className="mt-1 text-xs font-black text-slate-900">{card.titulo}</h4></div><div className="flex items-center gap-1.5"><button onClick={event => { event.stopPropagation(); openDerivedDemand(card); }} title="Criar tarefa relacionada para outro setor" className="grid h-7 w-7 place-items-center rounded-lg border border-amber-200 bg-amber-50 text-amber-800 hover:bg-amber-100"><Plus className="h-3.5 w-3.5" /></button>{card.room_number && <span className="rounded-lg bg-slate-950 px-2 py-1 text-[9px] font-black text-amber-300">Q. {card.room_number}</span>}</div></div>{card.descricao && <p className="mt-2 line-clamp-2 text-[10px] text-slate-500">{card.descricao}</p>}<div className="mt-3 grid grid-cols-2 gap-2 text-[9px]"><div className="rounded-lg bg-slate-50 p-2"><span className="text-slate-400">Responsável</span><p className="mt-0.5 truncate font-bold text-slate-700">{getGovernancaAssignedName(card) || 'Sem responsável'}</p></div><div className="rounded-lg bg-slate-50 p-2"><span className="text-slate-400">Atualizado</span><p className="mt-0.5 font-bold text-slate-700">{dateTime(card.updated_at)}</p></div></div>{demands.length > 0 && <div className="mt-3 rounded-xl border border-blue-100 bg-blue-50 p-2.5"><p className="text-[9px] font-black uppercase tracking-wide text-blue-600">Demandas relacionadas</p><div className="mt-1.5 space-y-1">{demands.map(demand => <div key={demand.id} className="flex items-center justify-between gap-2 text-[9px]"><span className="truncate font-bold text-slate-700">{demandSectorLabel(demand)} · {demand.titulo}</span><span className="shrink-0 rounded-md bg-white px-1.5 py-1 font-black text-blue-700">{demandStatusLabel(demand)}</span></div>)}</div></div>}<div className="mt-3 flex items-center justify-between gap-2"><span className="rounded-lg bg-slate-100 px-2 py-1 text-[9px] font-black uppercase text-slate-500">{card.prioridade || 'normal'}</span><span className="text-[9px] font-bold text-amber-700">Clique para detalhes e ações</span></div>{actionFor(card) && <div className="mt-3">{actionFor(card)}</div>}</article>; })}{columnCards.length === 0 && <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white/60 p-8 text-center"><Sparkles className="mx-auto h-5 w-5 text-slate-300" /><p className="mt-2 text-[10px] font-bold text-slate-400">Nenhuma tarefa nesta etapa</p></div>}</div></div>; })}</section>}
    </main>
  </div>;
};
