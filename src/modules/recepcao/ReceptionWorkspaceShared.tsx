...
import { AlertTriangle, BedDouble, CalendarDays, LogOut, Search, UserRound, Wifi } from 'lucide-react';
...
export const ReceptionWorkspaceShared: React.FC<{ definition: WorkspaceDefinition }> = ({ definition }) => {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-950">
    ...
    <main className="mx-auto max-w-[1600px] space-y-5 p-4 sm:p-6">
      <OperationalWorkCenter<PanelKey> sectorName="Recepção" accentClass="text-blue-600" summary={`${occupied} hóspedes no hotel`} items={workItems} activeKey={activePanel} onOpen={setActivePanel} onClose={() => setActivePanel(null)} panelTitle={panelTitle} renderPanel={renderPanel} />
      ...
      <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">{columns.map(column => <div key={column.id} className="rounded-3xl border border-slate-200 bg-white p-3"><div className="mb-3 flex items-center justify-between"><h2 className="text-xs font-black uppercase tracking-wide text-slate-700">{column.nome}</h2><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-500">{visibleCards.filter(card => card.column_id === column.id).length}</span></div><div className="space-y-3">{visibleCards.filter(card => card.column_id === column.id).map(card => { const mine = assignedUserId(card) === currentUser?.id; return <article key={card.id} className={`rounded-2xl border bg-slate-50 p-3 ${mine ? 'border-blue-300 ring-1 ring-blue-100' : 'border-slate-200'}`}><div className="mb-2 flex items-start justify-between gap-2"><span className="rounded-md bg-blue-50 px-2 py-1 text-[9px] font-black uppercase text-blue-700">Recepção</span>{mine && <span className="rounded-md bg-slate-950 px-2 py-1 text-[9px] font-black uppercase text-white">★ Minha tarefa</span>}</div><h3 className="text-xs font-black">{card.titulo}</h3>{card.guest_name && <p className="mt-1 text-[10px] font-bold text-slate-600"><UserRound className="mr-1 inline h-3 w-3" />{card.guest_name}</p>}{card.room_number && <p className="mt-1 text-[10px] font-bold text-blue-700">Quarto {card.room_number}</p>}<p className="mt-2 text-[10px] text-slate-400">{assignedName(card)}</p><label className="mt-3 block text-[9px] font-black uppercase tracking-wide text-slate-400">Status<select disabled={!currentUser?.id || savingId === card.id} value={card.column_id} onChange={event => void moveToColumn(card, event.target.value)} className="mt-1 h-9 w-full rounded-xl border border-slate-200 bg-white px-2 text-xs font-bold normal-case text-slate-700 disabled:opacity-50">{columns.map(item => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></label></article>; })}</div></div>)}</section>
    </main>
  </div>;
};
