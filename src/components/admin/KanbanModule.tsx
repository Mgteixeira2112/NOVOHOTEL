...
import { AlertCircle, Check, ChevronDown, Clock3, GripVertical, LayoutDashboard, Plus, RefreshCw, Wifi, WifiOff, X } from 'lucide-react';
import { useHotel } from '../../context/HotelContext';
import { kanbanV2, KanbanV2Board, KanbanV2Card, KanbanV2Column } from '../../services/kanbanV2';

const uid = () => crypto.randomUUID();
const departmentName = (value?: string) => ({ recepcao: 'Recepção', governanca: 'Governança', cozinha: 'Cozinha', manutencao: 'Manutenção', financeiro: 'Financeiro', almoxarifado: 'Almoxarifado' } as Record<string, string>)[value || ''] || value || 'Operação';

export const KanbanModule: React.FC = () => {
  const { hotelConfig, currentUser } = useHotel();
  const hotelId = String(hotelConfig?.id || '');
  ...
  const load = useCallback(async () => {
    if (!hotelId) { setError('Hotel não identificado.'); setLoading(false); return; }
    setLoading(true); setError('');
    try {
      const result = await kanbanV2.load(hotelId);
  ...
  useEffect(() => {
    if (!hotelId) return;
    return kanbanV2.subscribe(hotelId, {
      onInsert: card => setCards(current => current.some(c => c.id === card.id) ? current : [...current, card]),
      onUpdate: card => setCards(current => current.map(c => c.id === card.id && String(c.updated_at || '') < String(card.updated_at || '') ? card : c)),
  ...
  const createCard = async () => {
    if (!hotelId || !activeBoardId || !boardColumns[0] || !newTitle.trim()) return;
    setSaving(true); setError('');
    try {
      const card = await kanbanV2.createCard({ hotelId, boardId: activeBoardId, columnId: boardColumns[0].id, titulo: newTitle, descricao: newDescription, prioridade: newPriority, departamento: activeBoard?.departamento });
      setCards(current => current.some(c => c.id === card.id) ? current : [...current, card]);
      setNewTitle(''); setNewDescription(''); setNewPriority('normal'); setModalOpen(false);
    } catch (e: any) { setError(e?.message || 'Falha ao criar card.'); }
    finally { setSaving(false); }
  };

  const moveCard = async (card: KanbanV2Card, targetColumnId: string) => {
    if (card.column_id === targetColumnId || saving) return;
    setSaving(true); setError('');
    try {
      const persisted = await kanbanV2.moveCard(hotelId, card.id, targetColumnId);
      setCards(current => current.map(c => c.id === persisted.id ? persisted : c));
    } catch (e: any) { setError(e?.message || 'Falha ao mover card. A alteração não foi salva.'); }
    finally { setSaving(false); setDraggingId(null); }
  };
        ...
        <header className="rounded-3xl bg-white border border-slate-200 shadow-sm p-5 sm:p-6">
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-5">
            <div>
              ...
              <div className="flex gap-2 mt-4 text-xs text-slate-500"><span>{activeCount} tarefas</span><span>•</span><span>{doneCount} concluídas</span><span>•</span><span>{currentUser?.nome || 'Usuário'}</span></div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setModalOpen(true)} disabled={!activeBoardId || !boardColumns.length} className="h-11 px-4 rounded-xl bg-slate-950 hover:bg-slate-800 text-white font-bold text-sm flex items-center gap-2 disabled:opacity-40"><Plus className="w-4 h-4" /> Novo card</button>
            </div>
          </div>
        </header>
        ...
        {loading ? <div className="rounded-3xl bg-white border border-slate-200 p-12 text-center text-slate-500">Carregando dados reais do Supabase…</div> : boardColumns.length === 0 ? <div className="rounded-3xl bg-white border border-dashed border-slate-300 p-12 text-center"><h2 className="font-black text-slate-900">Nenhum quadro configurado</h2><p className="text-sm text-slate-500 mt-2">O Kanban agora não usa dados fictícios. Crie um quadro e colunas no banco para iniciar a operação.</p></div> :
          <main className="grid gap-4 overflow-x-auto pb-4" style={{ gridTemplateColumns: `repeat(${Math.max(boardColumns.length, 1)}, minmax(280px, 1fr))` }}>
            {boardColumns.map(column => {
              ...
              return <section key={column.id} onDragOver={e => e.preventDefault()} onDrop={() => draggingId && void moveCard(boardCards.find(c => c.id === draggingId)!, column.id)} className="min-h-[520px] rounded-3xl bg-white border border-slate-200 shadow-sm p-3.5">
