...
export const WorkspaceEditorModule: React.FC = () => {
  ...
  const createNew = async () => {
    if (saving) return;
    const sector: OperationalSectorId = 'recepcao';
    const created = createWorkspaceDefinition({ name: 'Novo Workspace', sector });
  ...
  return <div className="space-y-5">
    <div className="rounded-3xl border border-stone-200 bg-white p-5 sm:p-6 shadow-sm"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div><div className="flex items-center gap-2"><LayoutTemplate className="w-5 h-5 text-amber-600" /><h2 className="text-lg font-black">Fábrica de Workspaces</h2></div><p className="mt-1 text-xs text-stone-500">Crie ambientes operacionais, vincule setores e boards e personalize os widgets sem alterar o motor Kanban.</p><div className="mt-2 flex items-center gap-2 text-[10px] font-bold text-stone-500"><span>Hotel: {hotelId}</span><span>•</span>{syncSource === 'supabase' ? <span className="inline-flex items-center gap-1 text-emerald-700"><Cloud className="w-3.5 h-3.5" /> Sincronizado</span> : syncSource === 'local' ? <span className="inline-flex items-center gap-1 text-amber-700"><CloudOff className="w-3.5 h-3.5" /> Fallback local</span> : <span>Carregando configuração…</span>}</div></div><div className="flex flex-wrap gap-2"><button disabled={saving} onClick={() => void createNew()} className="h-10 px-4 rounded-xl border border-amber-300 bg-amber-50 text-xs font-black flex items-center gap-2 disabled:opacity-50"><Plus className="w-4 h-4" /> Novo</button><button disabled={saving} onClick={() => void duplicate()} className="h-10 px-4 rounded-xl border border-stone-200 text-xs font-bold flex items-center gap-2 disabled:opacity-50"><Copy className="w-4 h-4" /> Duplicar</button><button disabled={saving} onClick={() => void reset()} className="h-10 px-4 rounded-xl border border-stone-200 text-xs font-bold flex items-center gap-2 disabled:opacity-50"><RotateCcw className="w-4 h-4" /> {isBaseWorkspace ? 'Restaurar' : 'Remover'}</button><button disabled={saving} onClick={() => void save()} className="h-10 px-4 rounded-xl bg-stone-950 text-white text-xs font-black flex items-center gap-2 disabled:opacity-50"><Save className="w-4 h-4" /> {saving ? 'Salvando…' : 'Salvar'}</button></div></div>{message && <div className="mt-4 rounded-xl bg-emerald-50 border border-emerald-200 px-3 py-2 text-xs font-bold text-emerald-700">{message}</div>}</div>
    <div className="grid lg:grid-cols-[280px_1fr] gap-5"><aside className="rounded-3xl border border-stone-200 bg-white p-3 h-fit"><div className="mb-2 flex items-center justify-between px-2"><span className="text-[10px] font-black uppercase tracking-wider text-stone-400">Workspaces</span><span className="rounded-full bg-stone-100 px-2 py-1 text-[9px] font-black text-stone-500">{definitions.length}</span></div>{definitions.map(item => <button key={item.id} onClick={() => { setSelectedId(item.id); setMessage(''); }} className={`w-full text-left rounded-2xl p-3 transition ${selectedId === item.id ? 'bg-stone-950 text-white' : 'hover:bg-stone-100'}`}><div className="flex items-center justify-between gap-2"><p className="text-xs font-black truncate">{item.name}</p>{!workspaceRegistry.some(base => base.id === item.id) && <span className={`rounded px-1.5 py-0.5 text-[8px] font-black uppercase ${selectedId === item.id ? 'bg-amber-400 text-stone-950' : 'bg-amber-100 text-amber-800'}`}>Custom</span>}</div><p className={`mt-1 text-[10px] ${selectedId === item.id ? 'text-stone-300' : 'text-stone-500'}`}>{item.sectors.join(', ')}</p></button>)}</aside>
