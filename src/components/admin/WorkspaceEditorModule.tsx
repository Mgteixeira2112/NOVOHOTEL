...
export const WorkspaceEditorModule: React.FC = () => {
  ...
  const save = async () => { if (selected && !saving) await persistDefinition(selected, 'Workspace salvo no hotel e sincronizado com o Supabase.'); };
  const reset = async () => {
  ...
  return <div className="space-y-5">
    <div className="rounded-3xl border border-stone-200 bg-white p-5 sm:p-6 shadow-sm">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div><div className="flex items-center gap-2"><LayoutTemplate className="w-5 h-5 text-amber-600" /><h2 className="text-lg font-black">Fábrica de Workspaces</h2></div><p className="mt-1 text-xs text-stone-500">Monte ambientes operacionais por blocos, setores e boards sem alterar o motor Kanban.</p><div className="mt-2 flex items-center gap-2 text-[10px] font-bold text-stone-500"><span>Hotel: {hotelId}</span><span>•</span>{syncSource === 'supabase' ? <span className="inline-flex items-center gap-1 text-emerald-700"><Cloud className="w-3.5 h-3.5" /> Sincronizado</span> : syncSource === 'local' ? <span className="inline-flex items-center gap-1 text-amber-700"><CloudOff className="w-3.5 h-3.5" /> Fallback local</span> : <span>Carregando configuração…</span>}</div></div><div className="flex flex-wrap gap-2"><button disabled={saving} onClick={() => void createNew()} className="h-10 px-4 rounded-xl border border-amber-300 bg-amber-50 text-xs font-black flex items-center gap-2 disabled:opacity-50"><Plus className="w-4 h-4" /> Novo</button><button disabled={saving} onClick={() => void duplicate()} className="h-10 px-4 rounded-xl border border-stone-200 text-xs font-bold flex items-center gap-2 disabled:opacity-50"><Copy className="w-4 h-4" /> Duplicar</button><button disabled={saving} onClick={() => void reset()} className="h-10 px-4 rounded-xl border border-stone-200 text-xs font-bold flex items-center gap-2 disabled:opacity-50"><RotateCcw className="w-4 h-4" /> {isBaseWorkspace ? 'Restaurar' : 'Remover'}</button><button disabled={saving} onClick={() => void save()} className="h-10 px-4 rounded-xl bg-stone-950 text-white text-xs font-black flex items-center gap-2 disabled:opacity-50"><Save className="w-4 h-4" /> {saving ? 'Salvando…' : 'Salvar'}</button></div></div>
    ...
    <div className="grid lg:grid-cols-[280px_1fr] gap-5">
      <aside className="rounded-3xl border border-stone-200 bg-white p-3 h-fit"><div className="mb-2 flex items-center justify-between px-2"><span className="text-[10px] font-black uppercase tracking-wider text-stone-400">Workspaces</span><span className="rounded-full bg-stone-100 px-2 py-1 text-[9px] font-black text-stone-500">{definitions.length}</span></div>{definitions.map(item => <button key={item.id} onClick={() => { setSelectedId(item.id); setMessage(''); }} className={`w-full text-left rounded-2xl p-3 transition ${selectedId === item.id ? 'bg-stone-950 text-white' : 'hover:bg-stone-100'}`}><div className="flex items-center justify-between gap-2"><p className="text-xs font-black truncate">{item.name}</p>{!workspaceRegistry.some(base => base.id === item.id) && <span className={`rounded px-1.5 py-0.5 text-[8px] font-black uppercase ${selectedId === item.id ? 'bg-amber-400 text-stone-950' : 'bg-amber-100 text-amber-800'}`}>Custom</span>}</div><p className={`mt-1 text-[10px] ${selectedId === item.id ? 'text-stone-300' : 'text-stone-500'}`}>{item.sectors.join(', ')}</p></button>)}</aside>

      <section className="space-y-4">
        <div className="rounded-3xl border border-stone-200 bg-white p-5 grid sm:grid-cols-2 gap-4"><label className="text-xs font-bold text-stone-600">Nome<input value={selected.name} onChange={e => updateSelected({ name: e.target.value })} className="mt-2 w-full h-10 rounded-xl border border-stone-200 px-3 font-medium text-stone-900" /></label><label className="text-xs font-bold text-stone-600">Visão inicial<select value={selected.defaultScope} onChange={e => updateSelected({ defaultScope: e.target.value as WorkspaceDefinition['defaultScope'] })} className="mt-2 w-full h-10 rounded-xl border border-stone-200 px-3 bg-white text-stone-900"><option value="mine">Meu trabalho</option><option value="sector">Meu setor</option></select></label><label className="text-xs font-bold text-stone-600">Setor<select value={selectedSector} onChange={e => changeSector(e.target.value as OperationalSectorId)} className="mt-2 w-full h-10 rounded-xl border border-stone-200 px-3 bg-white text-stone-900">{OPERATIONAL_SECTORS.map(sector => <option key={sector.id} value={sector.id}>{sector.label}</option>)}</select></label><label className="text-xs font-bold text-stone-600">Board operacional<select value={selectedBoard} onChange={e => changeBoard(e.target.value)} className="mt-2 w-full h-10 rounded-xl border border-stone-200 px-3 bg-white text-stone-900">{WORKSPACE_BOARD_OPTIONS.map(board => <option key={board.id} value={board.id}>{board.label}</option>)}</select></label><label className="sm:col-span-2 text-xs font-bold text-stone-600">Descrição<input value={selected.description} onChange={e => updateSelected({ description: e.target.value })} className="mt-2 w-full h-10 rounded-xl border border-stone-200 px-3 font-medium text-stone-900" /></label>{!isBaseWorkspace && <div className="sm:col-span-2 flex justify-end"><button disabled={saving} onClick={() => void removeCustom()} className="inline-flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-black text-rose-700 disabled:opacity-50"><Trash2 className="w-4 h-4" /> Excluir Workspace personalizado</button></div>}</div>

        <div className="rounded-3xl border border-stone-200 bg-white p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-black text-stone-900">Biblioteca de widgets</h3><p className="mt-1 text-[10px] text-stone-500">Adicione blocos à composição. Eles só entram no Workspace após salvar.</p></div><span className="rounded-full bg-stone-100 px-2 py-1 text-[9px] font-black text-stone-500">{workspaceWidgetCatalog.length} tipos</span></div><div className="mt-4 space-y-4">{(['operacao','dados','equipe','atalhos'] as const).map(category => <div key={category}><p className="mb-2 text-[9px] font-black uppercase tracking-widest text-stone-400">{categoryLabels[category]}</p><div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-2">{workspaceWidgetCatalog.filter(item => item.category === category).map(item => <button key={item.type} onClick={() => addWidget(item.type)} className="group rounded-2xl border border-stone-200 p-3 text-left transition hover:border-amber-300 hover:bg-amber-50"><div className="flex items-center justify-between gap-2"><strong className="text-xs text-stone-800">{item.label}</strong><Plus className="w-3.5 h-3.5 text-stone-400 group-hover:text-amber-700" /></div><p className="mt-1 text-[10px] leading-relaxed text-stone-500">{item.description}</p></button>)}</div></div>)}</div></div>

        <div className="space-y-3"><div className="flex items-center justify-between px-1"><h3 className="text-sm font-black text-stone-900">Composição atual</h3><span className="text-[10px] font-bold text-stone-400">{selected.widgets.length} blocos</span></div>{[...selected.widgets].sort((a,b)=>(a.order??0)-(b.order??0)).map(widget => { const catalog = getWidgetCatalogItem(widget.type); return <div key={widget.id} className={`rounded-3xl border bg-white p-4 sm:p-5 ${widget.enabled === false ? 'border-stone-200 opacity-65' : 'border-amber-200 shadow-sm'}`}><div className="flex flex-col xl:flex-row xl:items-center gap-4"><div className="flex-1"><div className="flex items-center gap-2"><span className="rounded-lg bg-stone-100 px-2 py-1 text-[10px] font-black uppercase text-stone-600">{catalog?.label || widget.type}</span><strong className="text-sm">{widget.title || widget.id}</strong></div><p className="mt-1 text-[10px] text-stone-400">{widget.id}{widget.boardId ? ` • ${widget.boardId}` : ''}</p></div><div className="flex flex-wrap items-center gap-2"><select value={String(widget.span ?? catalog?.defaultSpan ?? 'full')} onChange={e => updateWidget(widget.id, { span: e.target.value === 'full' ? 'full' : Number(e.target.value) as WorkspaceWidgetSpan })} className="h-9 rounded-xl border border-stone-200 bg-white px-2 text-xs font-bold">{spanOptions.map(span => <option key={String(span)} value={String(span)}>Largura {span === 'full' ? 'total' : span}</option>)}</select><button onClick={() => moveWidget(widget.id, -1)} className="h-9 w-9 grid place-items-center rounded-xl border border-stone-200"><ArrowUp className="w-4 h-4" /></button><button onClick={() => moveWidget(widget.id, 1)} className="h-9 w-9 grid place-items-center rounded-xl border border-stone-200"><ArrowDown className="w-4 h-4" /></button><button onClick={() => updateWidget(widget.id, { enabled: widget.enabled === false })} className={`h-9 px-3 rounded-xl text-xs font-black flex items-center gap-2 ${widget.enabled === false ? 'bg-stone-100 text-stone-600' : 'bg-emerald-100 text-emerald-800'}`}>{widget.enabled === false ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}{widget.enabled === false ? 'Desativado' : 'Ativo'}</button><button onClick={() => removeWidget(widget.id)} className="h-9 w-9 grid place-items-center rounded-xl border border-rose-200 bg-rose-50 text-rose-700" title="Remover widget"><Trash2 className="w-4 h-4" /></button></div></div></div>; })}</div>
      </section>
    </div>
  </div>;
};
