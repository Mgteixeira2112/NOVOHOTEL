...
export const WorkspaceGeneralPresentationControls: React.FC<WorkspaceGeneralPresentationControlsProps> = ({ definition, onChange }) => {
  ...
  return <div className="rounded-3xl border border-stone-200 bg-white p-5" data-workspace-general-presentation>
    <div><h3 className="text-sm font-black text-stone-900">Apresentação do Workspace</h3><p className="mt-1 text-[10px] text-stone-500">A configuração geral é o padrão. Desktop, Mobile e KDS só usam override quando estiverem em Personalizar.</p></div>
    ...
    <div className="mt-4 rounded-2xl border border-stone-200 bg-stone-50 p-4">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-3">
        ...
        <label className="text-xs font-bold text-stone-600">Mobile<select value={mobileMode} onChange={e => updateDeviceMode('mobile', e.target.value as WorkspaceDevicePresentationMode)} className={fieldClass}><option value="auto">Adaptar automaticamente</option><option value="custom">Personalizar</option></select></label>
    ...
    <div className="mt-4">
      <p className="text-[9px] font-black uppercase tracking-wider text-stone-500">Cabeçalho operacional</p>
    ...
    {kdsMode !== 'disabled' && <div className="mt-5 border-t border-stone-200 pt-4">
      <p className="text-[9px] font-black uppercase tracking-wider text-stone-500">KDS / TV</p>
      <div className="mt-3 grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
        ...
        <label className="text-xs font-bold text-stone-600">Visualização<select value={kds.viewingDistance || 'medium'} onChange={e => updateKds({ viewingDistance: e.target.value as 'near' | 'medium' | 'far' })} className={fieldClass}><option value="near">Próxima</option><option value="medium">Média</option><option value="far">Longa</option></select></label>
