...
import { LayoutDashboard, BarChart3, BedDouble, CalendarDays, LogIn, Users, DollarSign, ShoppingBag, Bot, Palette, UserCheck, ShieldAlert, Lock, ArrowRight, Columns3, CreditCard, ChefHat, Sparkles, Layers, UtensilsCrossed, Briefcase, Sliders } from 'lucide-react';
import { AdminTab } from '../../types';
import { getTheme, getFontFamilyClass } from '../../utils/themeHelper';

type NavContextId = 'operacao' | 'vendas' | 'gestao' | 'sistema';
interface NavItemConfig { id: AdminTab; context: NavContextId; label: string; shortLabel?: string; icon: React.FC<{ className?: string }>; badge?: number; description?: string; }
...
export const AdminLayout: React.FC = () => {
  ...
  const navItems: NavItemConfig[] = useMemo(() => [
    ...
    { id: 'management_bi' as AdminTab, context: 'gestao', label: 'BI & KPIs Gerenciais', shortLabel: 'BI Gerencial', icon: BarChart3, description: 'ADR, RevPAR, Ocupação e Metas' },
    { id: 'users', context: 'gestao', label: 'Equipe & Acessos', shortLabel: 'Equipe', icon: UserCheck, badge: activeUsersCount, description: 'Usuários, setores e permissões RBAC' },
  ...
  React.useEffect(() => { if (currentTab && currentTab.context !== activeContext) setActiveContext(currentTab.context); }, [adminActiveTab]);
    ...
    <div className={`min-h-screen bg-stone-100/90 flex flex-col text-stone-900 ${fontClass}`}>
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white rounded-2xl border border-stone-200 p-2 shadow-xs mb-6 space-y-2">
          <div className="flex items-center justify-between border-b border-stone-100 pb-2 px-1 overflow-x-auto gap-1"><div className="flex items-center gap-1.5 min-w-max">{contexts.map((ctx) => { const Icon = ctx.icon; const isSelected = activeContext === ctx.id; const countActiveInCtx = navItems.filter((n) => n.context === ctx.id).reduce((acc, curr) => acc + (curr.badge || 0), 0); return <button key={ctx.id} onClick={() => { setActiveContext(ctx.id); const firstInContext = navItems.find((n) => n.context === ctx.id); if (firstInContext && currentTab.context !== ctx.id) setAdminActiveTab(firstInContext.id); }} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-2 transition cursor-pointer ${isSelected ? 'bg-stone-900 text-white shadow-xs' : 'text-stone-500 hover:text-stone-900 hover:bg-stone-100/80'}`}><Icon className={`w-3.5 h-3.5 ${isSelected ? 'text-amber-400' : 'text-stone-400'}`} /><span>{ctx.label}</span>{countActiveInCtx > 0 && <span className={`px-1.5 py-0.2 text-[9px] rounded-full font-black ${isSelected ? 'bg-amber-400 text-stone-950' : 'bg-stone-200 text-stone-700'}`}>{countActiveInCtx}</span>}</button>; })}</div><div className="hidden lg:flex items-center gap-1 text-[11px] text-stone-400 font-medium pr-2"><span>Módulo:</span><strong className="text-stone-700">{currentTab.label}</strong></div></div>
          <div className="overflow-x-auto pt-0.5"><nav className="flex items-center gap-1.5 min-w-max">{contextItems.map((item) => { const Icon = item.icon; const isActive = adminActiveTab === item.id || (item.id === 'settings' && adminActiveTab === 'design'); const isAllowed = item.id === ('command_center' as AdminTab) ? (userRole === 'admin' || userRole === 'gerente') : item.id === ('management_bi' as AdminTab) ? ['admin','gerente','financeiro'].includes(userRole) : hasTabAccess(userRole, item.id); return <button key={item.id} onClick={() => setAdminActiveTab(item.id)} className={`px-3.5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition cursor-pointer relative ${isActive ? `bg-stone-900 ${theme.textAccentClass} shadow-xs ring-1 ${theme.primaryBorder}` : isAllowed ? 'text-stone-600 hover:text-stone-950 hover:bg-stone-100/80' : 'text-stone-400 hover:text-stone-600 opacity-60'}`}><Icon className={`w-4 h-4 ${isActive ? theme.textAccentClass : isAllowed ? 'text-stone-400' : 'text-stone-300'}`} /><span>{item.label}</span>{!isAllowed && <Lock className="w-3 h-3 text-stone-400 ml-0.5" />}{item.badge !== undefined && item.badge > 0 && isAllowed && <span className={`px-2 py-0.2 text-[10px] rounded-full font-black ${isActive ? `${theme.primary} text-stone-950` : 'bg-stone-200 text-stone-700'}`}>{item.badge}</span>}</button>; })}</nav></div>
        </div>
        <main className="transition-all duration-200">
          {!hasPermission ? <div className="bg-white p-8 sm:p-12 rounded-3xl border border-stone-200 text-center max-w-xl mx-auto space-y-4 shadow-sm"><div className="w-14 h-14 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center mx-auto border border-amber-200"><ShieldAlert className="w-7 h-7" /></div><div className="space-y-1"><h3 className="font-bold text-lg text-stone-900">Acesso Restrito ao Módulo</h3><p className="text-xs text-stone-500 leading-relaxed">Seu perfil de <strong>{userRole.toUpperCase()}</strong> não possui autorização para gerenciar a aba <strong>{currentTab?.label}</strong>.</p></div><div className="pt-2 flex items-center justify-center gap-3"><button onClick={() => setAdminActiveTab('dashboard')} className={`px-4 py-2 rounded-xl ${theme.buttonClass} text-xs font-bold transition flex items-center gap-2 cursor-pointer`}><span>Retornar ao Dashboard Geral</span><ArrowRight className="w-3.5 h-3.5" /></button></div></div> : <><>{adminActiveTab === 'dashboard' && <DashboardModule />}{adminActiveTab === ('management_bi' as AdminTab) && <><ExecutiveDashboardModule /><DashboardAlertsWidget /></>}{adminActiveTab === ('command_center' as AdminTab) && <HotelOSCommandCenter />}{adminActiveTab === 'kanban' && <KanbanWorkspaceModule />}{adminActiveTab === 'reservations' && <ReservationsModule />}{adminActiveTab === 'checkin_out' && <CheckInOutModule />}{adminActiveTab === 'rooms' && <RoomsModule />}{adminActiveTab === 'guests' && <GuestsModule />}{adminActiveTab === 'financial' && <FinancialModule />}{adminActiveTab === 'frigobar' && <FrigobarModule />}{adminActiveTab === 'automation' && <AutomationModule />}{adminActiveTab === 'users' && <UsersOperationalAccessModule />}{adminActiveTab === 'pdv' && <PDVPage />}{adminActiveTab === 'kds' && <KDSPage />}{(adminActiveTab === 'settings' || adminActiveTab === 'design') && <SettingsModule />}</></>}
        </main>
      </div>
    </div>
  );
