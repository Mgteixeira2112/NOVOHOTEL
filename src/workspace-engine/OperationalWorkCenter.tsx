  ...
  return <>
    <section className="rounded-3xl border border-slate-200 bg-white p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <p className={`text-[10px] font-black uppercase tracking-wider ${accentClass}`}>Central de trabalho · tempo real</p>
      ...
      <div className="grid gap-2 md:grid-cols-3">
        {items.map(item => <button
          ...
          className="group flex min-h-[78px] items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-left transition hover:border-blue-300 hover:bg-blue-50/40"
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-baseline gap-2">
              ...
              <b className="text-lg">{item.value}</b>
            </span>
    ...
    >
      <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur">
          <div>
            <p className={`text-[10px] font-black uppercase tracking-wider ${accentClass}`}>Central de trabalho · {sectorName}</p>
            <h2 className="text-lg font-black">{panelTitle(activeKey)}</h2>
          </div>
