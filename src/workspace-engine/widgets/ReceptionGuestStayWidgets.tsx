...
import { Hospede, Reserva } from '../../types';
...
const tomorrow = () => tomorrowLocalDateKey();
const stayCheckin = (reservation: Reserva) => reservation.data_checkin || reservation.checkin || '';
const stayCheckout = (reservation: Reserva) => reservation.data_checkout || reservation.checkout || '';
...
export const ActiveStaysWidget: React.FC<WorkspaceWidgetRuntimeContext> = ({ widget }) => {
  ...
  const checkout = async (id: string) => { if (!window.confirm('Confirmar check-out desta hospedagem?')) return; setBusy(id); setError(''); try { await receptionStayService.checkout(id, currentUser?.id); const synced = await syncFromSupabase(); if (!synced.success) throw new Error(synced.message || 'Check-out concluído, mas a lista não conseguiu atualizar.'); } catch (e: any) { setError(e?.message || 'Não foi possível realizar o check-out.'); } finally { setBusy(null); } };
  return <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center gap-2"><BedDouble className="h-4 w-4 text-blue-600"/><div><h2 className="text-sm font-black">{widget.title || 'Hóspedes hospedados'}</h2><p className="text-[10px] text-slate-400">{active.length} hospedagem(ns) ativa(s)</p></div></div><div className="mt-3 grid gap-2 md:grid-cols-2">{active.map(r => { const g=guests.find(x=>x.id===r.hospede_id); const room=rooms.find(x=>x.id===r.quarto_id); return <div key={r.id} className="rounded-2xl border border-blue-100 bg-blue-50/40 p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><strong className="block truncate text-[11px]">{g?.nome || 'Hóspede'}</strong><p className="mt-1 text-[9px] text-slate-500"><CalendarDays className="mr-1 inline h-3 w-3"/>{stayCheckin(r)} → {stayCheckout(r)}</p></div><span className="rounded-lg bg-white px-2 py-1 text-[9px] font-black text-blue-700">Q. {room?.numero || '—'}</span></div><button disabled={busy===r.id} onClick={() => checkout(r.id)} className="mt-3 flex h-8 w-full items-center justify-center gap-1.5 rounded-lg bg-rose-50 text-[9px] font-black text-rose-700 disabled:opacity-50"><LogOut className="h-3.5 w-3.5"/>CHECK-OUT</button></div>; })}{active.length===0 && <p className="py-6 text-center text-xs text-slate-400">Nenhuma hospedagem ativa.</p>}</div>{error && <p className="mt-3 rounded-xl bg-rose-50 p-2 text-[10px] font-bold text-rose-700">{error}</p>}</div>;
};
