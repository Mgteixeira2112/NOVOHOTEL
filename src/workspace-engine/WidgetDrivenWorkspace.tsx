...
export const WidgetDrivenWorkspace: React.FC<WidgetDrivenWorkspaceProps> = ({ definition }) => {
  ...
  return <div className={`min-h-screen text-slate-950 ${isKds ? 'bg-slate-950 text-white' : 'bg-slate-100'}`} data-workspace-runtime="widget-driven" data-workspace-id={definition.id} data-workspace-viewport={viewport}>
    ...
    <main className={`mx-auto max-w-[1800px] p-4 sm:p-6 ${viewport === 'mobile' ? 'columns-1' : isKds ? 'grid grid-cols-1 gap-5 lg:grid-cols-3 xl:grid-cols-4' : 'columns-1 gap-4 md:columns-2 xl:columns-4'}`}>
      {widgets.map(widget => {
        ...
        return <section key={widget.id} className={`${isKds ? spanClass(widget.span) : 'mb-4 break-inside-avoid'} ${heightClass(widget)}`} data-workspace-widget={widget.type} data-widget-id={widget.id} data-widget-display={isButton ? 'button' : 'inline'}>
