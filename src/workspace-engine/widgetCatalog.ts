...
const allWorkspaceWidgetCatalog: WorkspaceWidgetCatalogItem[] = [
  ...
  { type: 'financial-overview', label: 'Visão Financeira Certificada', description: 'Posição financeira consolidada somente com receitas operacionais e contas administrativas oficiais, sem estimativas contábeis.', category: 'dados', requiresBoard: false, defaultSpan: 'full', defaultDataSource: 'finance', readiness: 'ready', readinessNote: 'Não se apresenta como DRE completa: exclui taxas estimadas, fallbacks e métricas sem contrato contábil oficial.', requiredRbacResource: 'financial' },
...
const kdsSuitability: Partial<Record<WorkspaceWidgetType, { suitability: WorkspaceWidgetKdsSuitability; reason: string }>> = {
  ...
  'financial-overview': { suitability: 'unsupported', reason: 'Posição financeira gerencial não deve aparecer automaticamente em monitores KDS compartilhados.' },
