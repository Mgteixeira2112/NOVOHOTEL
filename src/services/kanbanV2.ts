...
export const kanbanV2 = {
  ...
  async moveCard(_hotelId: string | undefined, cardId: string, columnId: string) {
    ...
    if (updatedCard) {
      ...
      if (roomNum) {
        ...
        if (colLower.includes('liberado') || (dept === 'governanca' && isDone) || colLower === 'man-col-resolvido') newRoomStatus = 'disponivel';
        else if (colLower.includes('limpeza') || colLower.includes('inspecao') || (dept === 'governanca' && colLower.includes('andamento'))) newRoomStatus = 'limpeza';
        else if (colLower.includes('a-limpar') || (dept === 'governanca' && colLower.includes('entrada'))) newRoomStatus = 'sujo';
        else if (colLower.includes('man-col') || dept === 'manutencao') newRoomStatus = (colLower.includes('resolvido') || isDone) ? 'disponivel' : 'manutencao';
        if (newRoomStatus && typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('pms_room_status_sync', { detail: { roomNumber: roomNum, status: newRoomStatus, sourceCardId: cardId } }));
      }
      return updatedCard;
    }
  ...
  async syncRoomStatus(roomNumber: string, status: string, details?: string) {
    ...
    let modified = false;
    if (status === 'sujo') {
      const existing = store.cards.find(c => (c.room_number === roomNumber || c.location?.includes(roomNumber)) && (c.board_id === 'kanban-board-governanca' || c.board_id === DEFAULT_BOARD_ID) && !c.column_id.includes('liberado') && !c.column_id.includes('concluido'));
      if (!existing) {
        store.cards.push({ id: `gov_card_${roomNumber}_${Date.now()}`, hotel_id: KANBAN_TENANT_ID, board_id: 'kanban-board-governanca', column_id: 'gov-col-a-limpar', titulo: `Higienização Quarto ${roomNumber}`, descricao: details || 'Quarto desocupado / necessita arrumação e higienização completa.', prioridade: 'atencao', ordem: Date.now(), departamento: 'governanca', room_number: roomNumber, location: `Quarto ${roomNumber}`, assigned_to: null, checklist: [{ id: '1', text: 'Troca de enxoval e toalhas', completed: false }, { id: '2', text: 'Limpeza de banheiro e desinfecção', completed: false }, { id: '3', text: 'Reposição de frigobar e amenities', completed: false }, { id: '4', text: 'Vistoria e liberação final', completed: false }], comments: [], metadata: { pms_synced: true }, completed_at: null, created_at: now, updated_at: now, is_archived: false, guest_name: null, reservation_id: null, service_details: null, tags: ['Governança', 'Higienização'], notes: null });
        modified = true;
      }
    } else if (status === 'limpeza') {
      store.cards = store.cards.map(c => { if ((c.room_number === roomNumber || c.location?.includes(roomNumber)) && (c.board_id === 'kanban-board-governanca' || c.departamento === 'governanca')) { modified = true; return { ...c, column_id: 'gov-col-em-limpeza', updated_at: now }; } return c; });
    } else if (status === 'manutencao') {
      const existing = store.cards.find(c => (c.room_number === roomNumber || c.location?.includes(roomNumber)) && (c.board_id === 'kanban-board-manutencao' || c.departamento === 'manutencao') && !c.column_id.includes('resolvido'));
      if (!existing) { store.cards.push({ id: `man_card_${roomNumber}_${Date.now()}`, hotel_id: KANBAN_TENANT_ID, board_id: 'kanban-board-manutencao', column_id: 'man-col-chamados', titulo: `Manutenção Quarto ${roomNumber}: ${details || 'Reparo Técnico'}`, descricao: details || 'Ordem de serviço aberta para manutenção do quarto.', prioridade: 'critica', ordem: Date.now(), departamento: 'manutencao', room_number: roomNumber, location: `Quarto ${roomNumber}`, assigned_to: null, checklist: [], comments: [], metadata: { pms_synced: true }, completed_at: null, created_at: now, updated_at: now, is_archived: false, guest_name: null, reservation_id: null, service_details: null, tags: ['Manutenção', 'Reparo'], notes: null }); modified = true; }
    } else if (status === 'disponivel') {
      store.cards = store.cards.map(c => { if ((c.room_number === roomNumber || c.location?.includes(roomNumber)) && !c.is_archived) { if (c.board_id === 'kanban-board-governanca' && !c.column_id.includes('liberado')) { modified = true; return { ...c, column_id: 'gov-col-liberado', completed_at: now, updated_at: now }; } if (c.board_id === 'kanban-board-manutencao' && !c.column_id.includes('resolvido')) { modified = true; return { ...c, column_id: 'man-col-resolvido', completed_at: now, updated_at: now }; } if (c.board_id === DEFAULT_BOARD_ID && !c.column_id.includes('concluido')) { modified = true; return { ...c, column_id: 'kanban-default-column-concluido', completed_at: now, updated_at: now }; } } return c; });
    }
    if (modified) saveLocalStore(store);
  },
  ...
  async syncReservation(res: { id: string; codigo: string; status: string; guestName: string; roomNumber?: string; total?: number; checkin?: string; checkout?: string }) {
    ...
    if (res.status === 'confirmada' || res.status === 'checkin_realizado') {
      ...
      if (!existing) { store.cards.push({ id: `rec_card_${res.id}_${Date.now()}`, hotel_id: KANBAN_TENANT_ID, board_id: 'kanban-board-recepcao', column_id: targetColumn, titulo: `${res.status === 'checkin_realizado' ? 'Hóspede In-House' : 'Check-in Previsto'}: ${res.guestName} (#${res.codigo})`, descricao: `Reserva #${res.codigo} | Quarto ${res.roomNumber || 'A definir'} | Check-out: ${res.checkout || 'N/D'}`, prioridade: 'normal', ordem: Date.now(), departamento: 'recepcao', room_number: res.roomNumber || null, location: res.roomNumber ? `Quarto ${res.roomNumber}` : 'Recepção', assigned_to: null, checklist: [], comments: [], metadata: { reservation_id: res.id, total: res.total }, completed_at: null, created_at: now, updated_at: now, is_archived: false, guest_name: res.guestName, reservation_id: res.id, service_details: null, tags: ['Recepção', res.status === 'checkin_realizado' ? 'In-House' : 'Check-in'], notes: null }); modified = true; }
      else if (existing.column_id !== targetColumn) { store.cards = store.cards.map(c => c.id === existing.id ? { ...c, column_id: targetColumn, updated_at: now } : c); modified = true; }
    } else if (res.status === 'checkout_concluido' || res.status === 'cancelada') {
      store.cards = store.cards.map(c => { if ((c.reservation_id === res.id || c.titulo.includes(res.codigo)) && !c.column_id.includes('finalizado')) { modified = true; return { ...c, column_id: 'rec-col-finalizado', completed_at: now, updated_at: now }; } return c; });
    }
    if (modified) saveLocalStore(store);
  },
  ...
  async syncMinibar(roomNumber: string, needsRestock: boolean, missingSummary?: string) {
    ...
    let modified = false;
    if (needsRestock) {
      const existing = store.cards.find(c => (c.room_number === roomNumber || c.location?.includes(roomNumber)) && c.titulo.toLowerCase().includes('frigobar') && !c.column_id.includes('concluido') && !c.column_id.includes('liberado'));
      if (!existing) { store.cards.push({ id: `mb_card_${roomNumber}_${Date.now()}`, hotel_id: KANBAN_TENANT_ID, board_id: 'kanban-board-governanca', column_id: 'gov-col-a-limpar', titulo: `Reposição Frigobar Quarto ${roomNumber}`, descricao: missingSummary || `Quarto ${roomNumber} necessita reposição de itens de frigobar.`, prioridade: 'atencao', ordem: Date.now(), departamento: 'governanca', room_number: roomNumber, location: `Quarto ${roomNumber}`, assigned_to: null, checklist: [], comments: [], metadata: { type: 'frigobar_restock' }, completed_at: null, created_at: now, updated_at: now, is_archived: false, guest_name: null, reservation_id: null, service_details: null, tags: ['Frigobar', 'Reposição'], notes: null }); modified = true; }
    } else {
      store.cards = store.cards.map(c => { if ((c.room_number === roomNumber || c.location?.includes(roomNumber)) && c.titulo.toLowerCase().includes('frigobar')) { modified = true; return { ...c, column_id: 'gov-col-liberado', completed_at: now, updated_at: now }; } return c; });
    }
    if (modified) saveLocalStore(store);
  },
