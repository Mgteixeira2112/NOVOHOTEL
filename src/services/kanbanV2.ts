...
export const kanbanV2 = {
  ...
  async moveCard(hotelId: string, cardId: string, columnId: string) {
    ...
    const { data, error } = await supabase
      .from('kanban_cards')
      .update({ column_id: columnId, updated_at: now })
      .eq('id', cardId)
      .eq('hotel_id', hotelId)
      .select('*')
      .single();
  ...
  async archiveCard(hotelId: string, cardId: string) {
    const { data, error } = await supabase
      .from('kanban_cards')
      ...
      .select('*')
      .single();
    if (error) throw errorMessage('[KANBAN ARCHIVE CARD]', error);
    return normalizeCard(data);
  },

  subscribe(hotelId: string, handlers: {
