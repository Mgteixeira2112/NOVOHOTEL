...
/**
 * Converte uma linha de banco de dados (kanban_cards) para a entidade TypeScript de domínio KanbanCard.
 * Suporta tanto campos diretos de tabela quanto metadados herdados em JSONB (retrocompatibilidade).
 */
export function mapDatabaseCardToKanbanCard(row: any): KanbanCard {
  if (!row) {
    throw new Error('mapDatabaseCardToKanbanCard: linha do banco de dados inválida ou nula');
  }

  const meta = row.metadata && typeof row.metadata === 'object' ? row.metadata : {};

  // Resolução de departamento
  ...
  // Resolução de descrição e detalhes do serviço
  let serviceDetails: string[] = [];
  if (Array.isArray(row.service_details) && row.service_details.length > 0) {
    serviceDetails = row.service_details.map(String);
  } else if (typeof row.descricao === 'string' && row.descricao.trim().length > 0) {
    serviceDetails = row.descricao.split('\n');
  } else if (Array.isArray(meta.service_details)) {
    serviceDetails = meta.service_details.map(String);
  }

  // Resolução de comentários
  const comments: KanbanCardComment[] = Array.isArray(row.comments) 
    ? row.comments 
    : (Array.isArray(meta.comments) ? meta.comments : []);

  // Resolução de checklist
  const checklist: KanbanChecklistItem[] = Array.isArray(row.checklist)
    ? row.checklist
    : (Array.isArray(meta.checklist) ? meta.checklist : []);
  ...
  if (row.assigned_to && typeof row.assigned_to === 'object') {
    assignedTo = row.assigned_to as KanbanCardAssignee;
  } else if (meta.assigned_to && typeof meta.assigned_to === 'object') {
    assignedTo = meta.assigned_to as KanbanCardAssignee;
  }
...
/**
 * Converte a entidade de domínio KanbanCard para o formato de persistência PostgreSQL na tabela kanban_cards.
 */
export function mapKanbanCardToDatabaseRow(card: KanbanCard, hotelId: string): Record<string, any> {
  ...
  const descricao = serviceDetails.length > 0 ? serviceDetails.join('\n') : null;
...
/**
 * Converte uma linha de banco de dados (kanban_columns) para a entidade TypeScript de domínio KanbanColumn.
 */
export function mapDatabaseColumnToKanbanColumn(row: any): KanbanColumn {
  ...
  return {
    id: String(row.id),
    board_id: String(row.board_id),
    title: String(row.nome || row.title || 'Coluna'),
    order: Number(row.ordem ?? row.order ?? 0),
    color: row.cor || config.color || '#64748b',
    wip_limit: row.wip_limit !== null && row.wip_limit !== undefined ? Number(row.wip_limit) : (config.wip_limit !== undefined ? Number(config.wip_limit) : undefined),
    is_final: Boolean(row.is_final ?? config.is_final ?? false),
    is_in_progress: Boolean(row.is_in_progress ?? config.is_in_progress ?? false),
    is_delegated: Boolean(row.is_delegated ?? config.is_delegated ?? false),
  };
}

/**
 * Converte a entidade de domínio KanbanColumn para o formato de persistência PostgreSQL na tabela kanban_columns.
 */
...
/**
 * Converte uma linha de banco de dados (kanban_boards) e suas colunas associadas para a entidade KanbanBoard.
 */
export function mapDatabaseBoardToKanbanBoard(row: any, columns: KanbanColumn[] = []): KanbanBoard {
  const config = row.configuracao && typeof row.configuracao === 'object' ? row.configuracao : {};

  const boardColumns = columns
    .filter((col) => col.board_id === row.id)
    .sort((a, b) => a.order - b.order);

  return {
    id: String(row.id),
    title: String(row.nome || row.title || 'Quadro'),
    department: String(row.departamento || row.department || 'geral'),
    icon_name: String(row.icon_name || config.icon_name || 'Layers'),
    description: String(row.descricao || row.description || ''),
    default_sla_minutes: Number(row.default_sla_minutes || config.default_sla_minutes || 60),
    allowed_roles_manage: Array.isArray(row.allowed_roles_manage) 
      ? row.allowed_roles_manage 
      : (Array.isArray(config.allowed_roles_manage) ? config.allowed_roles_manage : ['admin', 'gerente']),
    allowed_roles_view: Array.isArray(row.allowed_roles_view) 
      ? row.allowed_roles_view 
      : (Array.isArray(config.allowed_roles_view) ? config.allowed_roles_view : ['todas']),
    columns: boardColumns,
    is_custom: Boolean(row.is_custom ?? config.is_custom ?? false),
  };
}

/**
 * Converte a entidade de domínio KanbanBoard para o formato de persistência PostgreSQL na tabela kanban_boards.
 */
