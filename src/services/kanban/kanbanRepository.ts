...
export const kanbanRepository = {
  ...
  async updateCard(hotelId: string, card: KanbanCard): Promise<KanbanCard> {
    ...
    if (!data) throw new Error(`[SUPABASE CONCURRENCY ERROR] Card ${card.id}: o registro foi alterado por outro cliente ou a versão local está desatualizada.`);
  ...
  async upsertCard(hotelId: string, card: KanbanCard): Promise<KanbanCard> {
    ...
    const { data: existing, error: lookupError } = await supabase.from('kanban_cards').select('id, updated_at').eq('id', card.id).eq('hotel_id', hotelId).maybeSingle();
    if (lookupError) {
