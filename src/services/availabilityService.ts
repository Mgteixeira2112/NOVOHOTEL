...
export const availabilityService = {
  async search(input: AvailabilitySearch) {
  ...
  async calculatePrice(input: Parameters<typeof availabilityRepository.calculatePrice>[0]) {
    if (input.checkout <= input.checkin) throw new Error('Check-out deve ser posterior ao check-in');
    return availabilityRepository.calculatePrice(input);
  },
  async createHold(input: Parameters<typeof availabilityRepository.createHold>[0]) {
    if (input.checkout <= input.checkin) throw new Error('Check-out deve ser posterior ao check-in');
