...
export const DeviceProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const value = useMemo(() => ({ device, setDevice, clearDeviceSession: () => setDevice(null) }), [device]);
  return <DeviceContext.Provider value={value}>{children}</DeviceContext.Provider>;
};

export function useDevice() {
  const value = useContext(DeviceContext);
  if (!value) throw new Error('useDevice deve ser utilizado dentro de DeviceProvider');
  return value;
}
