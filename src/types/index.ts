...
export interface Reserva {
  ...
  origem?: 'direto' | 'booking' | 'airbnb' | 'expedia' | 'telefone' | 'outro' | string;
  valor_total?: number;
...
export interface Pagamento {
  ...
  reserva_id: string;
  valor: number;
  metodo: 'pix' | 'cartao_credito' | 'cartao_debito' | 'dinheiro' | 'transferencia' | string;
...
export interface BloqueioQuarto {
  ...
  data_inicio: string;
  data_fim: string;
  tipo?: 'manutencao' | 'bloqueio' | 'evento' | string;
  ativo?: boolean;
  criado_por?: string;
  created_at?: string;
}
...
export interface AutomacaoMensagem {
  id: string;
  titulo?: string;
  tipo?: 'pre_checkin' | 'confirmacao_reserva' | 'pos_checkout' | 'senha_fechadura' | 'lembrete_pagamento' | string;
  nome?: string;
  gatilho: string;
  dias_offset?: number;
  canal?: 'whatsapp' | 'email' | 'ambos' | string;
  ativo?: boolean;
  assunto?: string;
  template?: string;
...
export interface HotelConfigSobreDiferencial {
  titulo: string;
  desc: string;
}

export interface HotelComodidadePersonalizada {
  id: string;
  icone: string;
  titulo: string;
  descricao: string;
...
export interface HotelDepoimento {
  id: string;
  nome: string;
  origem: string;
  avaliacao: number;
...
export interface HotelPontoInteresse {
  id: string;
  nome: string;
  distancia: string;
  tipo: 'lazer' | 'gastronomia' | 'educacao' | 'negocios' | 'transporte' | 'outro' | string;
}
...
export interface HotelSecoesVisibilidade {
  show_hero?: boolean;
...
export interface HotelRedesSociais {
  instagram?: string;
  facebook?: string;
  tripadvisor?: string;
  booking?: string;
...
export interface HotelConfig {
  ...
  depoimentos: HotelDepoimento[];
  faqs: HotelFaq[];
  pontos_interesse: HotelPontoInteresse[];
  ...
  redes_sociais: HotelRedesSociais;
...
export interface SecurityLogEntry {
  ...
  details?: string;
  detalhes?: string;
  status?: 'sucesso' | 'bloqueado' | 'alerta' | string;
  sucesso?: boolean;
}
...
export interface HotelOSTask {
  ...
  description?: string;
  department: string;
  status: HotelOSTaskStatus;
  priority: HotelOSTaskPriority;
  room_id?: string;
...
export interface HotelOSWorkflow {
  id: string;
  hotel_id?: string;
  name: string;
  description?: string;
