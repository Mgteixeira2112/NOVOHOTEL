...
export interface Reserva { id: string; codigo?: string; codigo_reserva?: string; hospede_id: string; quarto_id: string; data_checkin?: string; data_checkout?: string; checkin?: string; checkout?: string; checkin_horario?: string; checkout_horario?: string; adultos?: number; criancas?: number; quantidade_hospedes?: number; status: ReservationStatus; origem?: 'direto' | 'booking' | 'airbnb' | 'expedia' | 'telefone' | 'outro' | string; valor_total?: number; valor_diarias?: number; valor_taxas?: number; valor_consumo?: number; valor_pago?: number; forma_pagamento?: string; pagamento_id?: string; observacoes?: string; consumo_itens?: ConsumoExtra[]; pin_fechadura?: string; created_at?: string; }
export interface Pagamento { id: string; reserva_id: string; valor: number; metodo: 'pix' | 'cartao_credito' | 'cartao_debito' | 'dinheiro' | 'transferencia' | string; status: 'pendente' | 'aprovado' | 'recusado' | 'estornado'; data_pagamento?: string; codigo_transacao?: string; transacao_id?: string; parcelas?: number; observacoes?: string; created_at?: string; }
