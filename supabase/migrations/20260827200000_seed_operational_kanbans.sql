...
-- Não remove dados existentes e pode ser executada mais de uma vez.
...
-- Garante que o Realtime continue habilitado após a criação/atualização dos registros.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'kanban_boards'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.kanban_boards;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'kanban_columns'
  ) THEN
