-- Seed idempotente dos Kanbans operacionais.
-- Todos os quadros usam as mesmas tabelas persistentes e o mesmo Supabase Realtime.
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='kanban_boards') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.kanban_boards;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='kanban_columns') THEN
