...
alter table public.hotel_os_folio_items add column if not exists source_key text;
alter table public.hotel_os_transactions add column if not exists idempotency_key text;
...
create or replace function public.hotel_os_financial_add_charge(p_folio_id uuid,p_source text,p_source_key text,p_description text,p_quantity numeric,p_unit_price numeric)
...
begin
  ...
  perform public.hotel_os_require_stay_permission(f.hotel_id,case p_source when 'DISCOUNT' then 'discount' when 'ADJUSTMENT' then 'folio_adjustment' else 'folio_item_add' end);
  ...
  v_item_type:=case p_source when 'ROOM' then 'room' when 'POS' then 'order' when 'FRIGOBAR' then 'minibar' when 'TAX' then 'tax' when 'DISCOUNT' then 'adjustment' when 'ADJUSTMENT' then 'adjustment' else 'other' end;
  insert into public.hotel_os_folio_items(hotel_id,folio_id,item_type,source,source_key,description,quantity,unit_amount,total,status,created_by)
  values(f.hotel_id,p_folio_id,v_item_type,p_source,nullif(trim(coalesce(p_source_key,'')),''),p_description,p_quantity,p_unit_price,round(p_quantity*p_unit_price,2),'active',auth.uid()) returning id into v_id;
  perform public.hotel_os_emit_event(f.hotel_id,'folio.item_added','FOLIO','Folio',f.id,jsonb_build_object('folio_item_id',v_id,'source',p_source,'source_key',p_source_key,'total',round(p_quantity*p_unit_price,2)),auth.uid());
  return v_id;
end; $$;
...
create or replace function public.hotel_os_financial_receive_payment(p_folio_id uuid,p_amount numeric,p_method text,p_external_reference text default null,p_idempotency_key text default null)
returns uuid language plpgsql security definer set search_path=public as $$
declare f record; v_existing uuid; v_id uuid;
begin
  select * into f from public.hotel_os_folios where id=p_folio_id for update;
  if not found then raise exception 'Folio não encontrado'; end if;
  ...
  if nullif(trim(coalesce(p_idempotency_key,'')),'') is not null then update public.hotel_os_transactions set idempotency_key=p_idempotency_key where id=v_id; end if;
  return v_id;
end; $$;
...
begin
  ...
  select coalesce(sum(case when status='active' then total else 0 end),0),coalesce(jsonb_agg(jsonb_build_object('id',id,'source',source,'sourceKey',source_key,'description',description,'quantity',quantity,'unitPrice',unit_amount,'total',total,'status',status,'createdAt',created_at) order by created_at) filter(where id is not null),'[]'::jsonb)
    into v_charges,v_items from public.hotel_os_folio_items where folio_id=f.id;
  select coalesce(sum(case when transaction_type='payment' and status='approved' then amount else 0 end),0),coalesce(sum(case when transaction_type='refund' and status in ('approved','refunded') then amount else 0 end),0),coalesce(jsonb_agg(jsonb_build_object('id',id,'amount',amount,'method',coalesce(payment_method,method),'status',status,'externalReference',external_reference,'idempotencyKey',idempotency_key,'createdAt',created_at) order by created_at) filter(where transaction_type='payment'),'[]'::jsonb)
    into v_payments,v_refunds,v_payment_rows from public.hotel_os_transactions where folio_id=f.id;
  return jsonb_build_object('folioId',f.id,'hotelId',f.hotel_id,'stayId',f.stay_id,'status',f.status,'currency',coalesce(f.currency,'BRL'),'chargesTotal',round(v_charges,2),'paymentsTotal',round(v_payments,2),'refundsTotal',round(v_refunds,2),'balance',round(v_charges-v_payments+v_refunds,2),'items',v_items,'payments',v_payment_rows);
end; $$;
...
begin select id into v_folio from public.hotel_os_folios where stay_id=p_stay_id order by created_at desc limit 1; if v_folio is null then raise exception 'Folio não encontrado'; end if; return public.hotel_os_financial_folio_snapshot(v_folio); end; $$;
