...
begin
  ...
  select c.column_id into v_existing
  ...
  select c.column_id into v_existing
   ...
   where c.board_id = 'kanban-board-manutencao'
     ...
     and c.column_id in ('man-col-chamados', 'man-col-reparo', 'man-col-pecas')
   order by case c.column_id when 'man-col-pecas' then 3 when 'man-col-reparo' then 2 else 1 end desc,
            c.updated_at desc
   limit 1;
...
begin
  ...
  -- Existing Governança/Kanban triggers can update quartos while a projection is being
  -- synchronized. Do not recursively start another projection pass from that nested update.
...
-- Backfill all current rooms. Existing room-gov-* records are upgraded in place.
do $$
declare
  v_room record;
begin
