...
DECLARE
  v_hotel_id UUID; v_status TEXT; v_permitido BOOLEAN := FALSE;
BEGIN
  SELECT hotel_id, status INTO v_hotel_id, v_status
  FROM public.pdv_pedidos WHERE id = p_pedido_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Pedido não encontrado'; END IF;
  IF NOT public.usuario_pode_hotel(v_hotel_id) THEN RAISE EXCEPTION 'Usuário sem acesso ao hotel'; END IF;
  IF p_novo_status NOT IN ('recebido','em_preparo','pronto','entregue','cancelado','fechado') THEN
  ...
  v_permitido :=
    ...
    (v_status = 'pronto' AND p_novo_status = 'entregue') OR
    (v_status = 'entregue' AND p_novo_status IN ('fechado','entregue'));

  IF NOT v_permitido THEN
    RAISE EXCEPTION 'Transição de status não permitida: % -> %', v_status, p_novo_status;
  END IF;

  UPDATE public.pdv_pedidos SET status = p_novo_status, atualizado_em = NOW() WHERE id = p_pedido_id;
  RETURN TRUE;
END;
$$;
